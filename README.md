...
- **星标**（点行尾 ★）：标记"我要死磕"的词——抽中频率 ×3，且满了 5 分也**不会**被当作已掌握、不会被一笔带过；星标词不计入今日进度，适合单独长期磨的词
...
- **拼写纠错**：本地和联网都查不到时（比如把 podcast 打成 `podcst`），给出"你是不是想找"建议，直接点开即可看详情或加入我要背
- 本地无精确匹配时**自动联网查询**兜底（有道词典公开接口，无 key），在线结果**置顶**显示，可一键加入"我要背"，也能**点开看完整详情**
- 详情页含音标、完整释义、真题词频、词组标签

### 我要背
- 全部 / 新词（只添加还没背）/ 学习中（未满 5 分）/ 已掌握（满 5 分）/ **星标**筛选
- 按添加时间倒序，**新加的排在最上面**；列表右侧有细滚动条，长列表一眼看得出位置
...
- 已在"我要背"的词标注**「已添加」**且不可勾选，杜绝重复添加
- **词黑名单**：行尾 🚫 把 the、a、an、date 这类噪声词拉黑，之后不再出现在扫词候选与词库搜索里；误拉黑可在"我的 → 生词管理 → 词黑名单"恢复
- 行尾统一 **🚫 黑名单 ｜ ★ 星标 ｜ + 加入我要背** 三个图标，尺寸一致；点按有回弹反馈
- 加错了？**再点一次 ✓ 即可撤回**（加入是弹一下、撤回是缩一下）
...
- **新增词黑名单**：行尾 🚫 把 the、a、an、date 这类噪声词拉黑，不再出现在扫词候选与词库搜索里；「我的 → 生词管理 → 词黑名单」可查看与恢复
- **新增星标**：标重点词，抽中频率 ×3，且满了 5 分也不算已掌握、一直高频出现；未加入"我要背"时点星标会自动加入
- **新增联网拼写纠错**：查不到时给出"你是不是想找"，如 `podcst` → podcast
- **新增**：词库在线结果可点开看完整详情；「我的 → 语音打开」引导你在小爱同学里建自定义指令
- **新增**：所有可下拉界面右侧一条细滚动条（深浅色自动适配）
- **交互**：加入按钮再点一次即撤回（加入弹一下、撤回缩一下）；行尾三个图标都有回弹动画；拉黑提示固定 3 秒
- **修复**：拉黑后有时仍留在列表上；已添加到"我要背"的词有时错误显示为未添加
- **调整**：「我要背」按添加时间倒序，新加的排最上面
- **修复**：版本号此前一直显示 0.1.0，现与发布版本一致
...
小爱同学说"打开记英语"经常听不准：它对"打开X"只按**应用名**匹配，第三方应用没有接口能注册语音别名或触发词（这属于系统侧能力，App 改不了）。可靠的替代做法：

1. 对小爱同学说"训练小爱"（或进小爱同学 → 更多 → 自定义指令）
2. 触发词填 **背单词**，动作选 **打开链接**
3. 在 App"我的 → 语音打开"里点"② 复制快捷方式"，把复制到的地址粘贴进去

之后说"背单词"即可唤起。App 内也支持 `rememberenglish://study`（直接开始背）与 `rememberenglish://add`（打开扫词）两条快捷方式。
...
- OCR 使用 Google ML Kit（离线模型）；联网查词仅在本地词库未收录时调用有道词典公开接口兜底；拼写纠错同样只在查不到时调用有道公开的拼写建议接口。
