...
- 作答后中文释义**停留展示**：答对停留 1.5 秒确认背没背对，答错/不清楚停留 3 秒看清意思（点击卡片可跳过）再进下一个词
...
- 本地无精确匹配时**自动联网查询**兜底（有道词典公开接口，无 key），在线结果**置顶**显示，可一键加入"我要背"
...
- 识别出的**未收录词自动联网查询释义**（实时扫词与拍照/相册均是，会话内缓存避免重复请求）
- 已在"我要背"的词标注**「已添加」**且不可勾选，杜绝重复添加；误识别的伪词（如 date/sun）可点行尾 **✕ 忽略**，之后不再出现（可撤销）
- 候选词排序：**真题词频**降序 → 词库命中 → 有联网释义（在线）→ 纯自定义词；未命中词库的作为自定义词入库

### 督促提醒
- 每日固定时间（默认 20:00 可设置）检查当日目标完成度，未完成则发送 heads-up 系统通知督促（含进度条与"去背单词"快捷按钮）；已完成或当天已提醒过则静默
- 通知点击直达学习页；Android 13+ 动态申请通知权限
- 收不到提醒？"我的"页提醒卡内提供**可靠性引导**：允许自启动、省电策略设为"无限制"、允许悬浮通知（各家手机管家清理后台后会拦截提醒通知）
...
- **修复**联网查词：适配有道词典接口新结构（此前联网查到的释义显示为空）
- **新增**实时扫词识别出的未收录词自动联网查释义（与拍照/相册一致，会话内缓存防刷请求）
- **新增**词库本地无精确匹配时也联网兜底，在线结果置顶、样式与本地结果统一
- **新增**扫词候选「已添加」标注（已在我要背的词不可重复勾选）；行尾 ✕ 忽略伪词（如 date/sun，可撤销）
- **新增**有联网释义的候选词排在纯自定义词之上
- **新增**背单词答对后展示中文释义 1.5 秒确认背没背对（答错/不清楚仍 3 秒）
- **新增**提醒通知进度条与「去背单词」快捷按钮；「我的」页提醒可靠性引导（自启动 / 省电策略 / 悬浮通知）
- **优化**页面切换动画提速（约 700ms → 180ms）

历史版本见 [Releases](https://github.com/qyf041107/Remember-English/releases)。
